...
import { POST_DATA } from "../graphQL/mutations";
    ...
    submitForm() {
      ...
      this.$apollo.mutate({
        ...
        update: (store, { data: { postData }}) => {
          if(!postData) throw new Error('Data wasn\'t saved!!');
          const data = store.readQuery({ query: GET_DATA })
          data.getData.push(newItem)
          store.writeQuery({ query: GET_DATA, data })
        },
      })
    },
  },
